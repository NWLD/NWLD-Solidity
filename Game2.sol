...
contract Game2 is Role {
    ...
    uint256 expReward = 20;
    uint256 tokenReward = 10000 * 10 ** 18;
    ...
    function fight(uint256 tokenId, uint256 count) external {
        ...
        uint256 fightRate = petFight * petFight / 2000 + petFight / 2;
        uint256 winCount = randomFight(fightRate, count, tokenId);
        //奖励代币
        uint256 winToken = winCount * tokenReward;
        //奖励经验
        uint256 exp = count * expReward;
        data.addLabelAddressData("game2Count", msg.sender, count);
        data.addLabelTokenData("exp", tokenId, exp);
        //赢的次数大于0
        if (0 < winCount) {
            data.addLabelAddressData("game2WinCount", msg.sender, winCount);
    ...
    function getFightPets(address owner) external view returns (uint256[] memory, uint256[] memory, uint256[] memory){
        ...
        for (uint256 index = 0; index < tokenIds.length; index++) {
            ...
            fightRates[index] = petFight * petFight / 2000 + petFight / 2;
        }
